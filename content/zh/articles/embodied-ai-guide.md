...
description: "本科毕业、三年嵌入式经验，能不能转进具身智能？这篇从要补的数学与代码基础、值得跟着复现的开源项目、怎么搭一个能拿得出手的作品集、面试常见考察点，以及什么样的公司和岗位更容易接受转岗者，给一份工程师视角、按季度排的转型路线图。"
toc: true
related_articles:
  - world-model-intro
...
- Python编程：不需要精通，但要能熟练写数据处理、模型训练、可视化脚本 
- 深度学习基础：理解CNN、RNN、Transformer的基本原理，能用PyTorch搭建和训练模型 
- 强化学习基础：理解MDP、策略梯度、Actor-Critic、Q-learning等核心概念 
- 线性代数和概率论：矩阵运算、概率分布、贝叶斯定理——这些是理解论文的数学基础 
...
- 仿真与Sim-to-Real：掌握一个仿真器（推荐MuJoCo），理解域随机化、系统辨识等迁移方法 
- 世界模型：理解RSSM、DreamerV3的架构，能跑通开源实现并做修改 
- 操作规划：理解运动规划（RRT、优化方法）、抓取规划、力控等 
- 感知与视觉：理解6D位姿估计、点云处理、视觉伺服等 
...
- 课程：李宏毅的机器学习课程（B站有，免费，讲得非常好） 
- 课程：David Silver的强化学习课程（YouTube/B站都有） 
- 实践：用PyTorch实现一个简单的DQN，在CartPole上跑通 
...
- 安装MuJoCo和对应的Python绑定 
- 跟着教程让一个机械臂学会 reaching（抓取目标位置） 
- 用PPO或SAC训练一个简单任务，理解训练过程中的各种参数 
- 推荐资源：MuJoCo官方文档和示例 
...
- 读DreamerV3论文（先读一遍不求甚解，再精读关键章节） 
- 找到开源实现（GitHub上搜"dreamerv3"），跑通Atari或MuJoCo的示例 
- 尝试修改一些超参数，观察对训练的影响 
- 尝试在自己的MuJoCo任务上使用DreamerV3 
...
- 选择一个具体任务（比如机械臂抓取、四足行走） 
- 在仿真中训练策略 
- 实现域随机化，测试策略的鲁棒性 
- 如果有硬件，尝试迁移到真实机器人；如果没有，做一个详细的仿真分析报告也行 
- 把过程和结果写成博客或论文 
