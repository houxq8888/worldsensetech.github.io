...
**"同一条 $\pi$"有前提**——sim 与 real 必须**共享同一 policy interface**：observation schema（键 / shape / 单位 / 归一化）、action schema（连续 or 离散、力矩 / 速度 / 位置、clamping）、control freq / action hold / delay 假设。interface 不一致、$\pi$ 非同一函数、$\delta_J$ 失去定义。
...
两类不能相加：reality mismatch 是"仿真与真实非同一世界"、task-spec mismatch 是"优化目标与部署目标非同一任务"。**观测与状态估计单独成层**——$a_t = \pi(o_t),\ o_t = h(x_t) + \epsilon$，camera 标定 / depth bias / 遮挡 / proprioception drift / 力传感偏置 / estimator 时延**让 policy 看到的 state 与 sim 假设 state 不一致**、manipulation / locomotion 里比外观 gap 更伤 performance。**Stochasticity mismatch**（motor 随机性 / friction variability / sensor temporal correlation / comm jitter / unmodeled disturbance / repeated-reset variability）是 dynamics 高阶统计量差异、正是 DR 覆盖对象。**Timing mismatch**（$\Delta t_{\mathrm{sim}} \neq \Delta t_{\mathrm{real}}$、action hold、sensor delay、inference latency、异步 observation）**可被闭环动力学放大**、非 additive、可改变 closed-loop stability。**Initial-state / env mismatch** $p_{\mathrm{train}}(s_0) \neq p_{\mathrm{eval}}(s_0)$；**Objective shift** $R_{\mathrm{train}} \neq R_{\mathrm{eval}}$ 下文默认已对齐。
...
**每个 $\Delta_k$ 是 mismatch descriptor、可 scalar / vector / distribution / set-valued**；$F$ 只是 schematic、标记"存在某种未展开的依赖"、由 sensitivity experiments / ablation 探测局部响应、**不是待估 predictive model**。**四个 $\Delta_k$ 是便于归因的 diagnostic buckets、非世界本身的正交 latent variables**——actuator delay 可伪装成 obs、estimator lag 可伪装成 ctrl、contact stochasticity 可伪装成 dynamics、bucket 既不正交也不可唯一辨识。层级显式写为 **reality / task discrepancies → diagnostic buckets（$\Delta_k$ 在此层）→ observable evidence → intervention candidates**、$F(\cdot)$ 只是归因链上一环、不是 latent factor model。
...
项目预算**非同一种货币**（GPU 近乎无限 / 真机机时稀缺 / 有机器时间却没工程人力）、正解是多预算 $C_{\mathrm{real}} \le B_{\mathrm{real}}$、$C_{\mathrm{compute}} \le B_{\mathrm{compute}}$、$C_{\mathrm{eng}} \le B_{\mathrm{eng}}$、不折成标量 $B$。**安全不进同一层 cost**——它是 chance constraint（$\alpha$ 由 e-stop / hardware fault 上限决定）、不是可折价软预算。**本文默认 deployment $J$ 已固定单一 utility 或已完成外部 scalarization**；若保留多目标（success / energy / cycle time / wear / safety）、$\Delta J$ / $Q_\lambda$ / $MV$ 应上升到 **Pareto 或 lexicographic layer**、不硬折进未说明的 $J$。
...
于是"下一块钱花在哪"是以干预为变量、需在真实世界逐步估计的量。**本文 decision framework 围绕两个量分层：局部 decision score $Q_\lambda$ 与效率读数 $MV$**——$Q_\lambda$ 是 $\arg\max$ 对象、$MV$ 是 cost-normalized 效率读数、跨预算 / intervention 比较、**不是 decision rule**。二者 conditional on allocation state $s_t = (b_t, \pi_t, \mathcal{D}_t, h_t)$——$h_t$ = hardware condition（battery / temperature / calibration / wear / mode）、显著非平稳时显式纳入、漂移小吸进 $\mathcal{D}_t$。**「DR 的 $MV$ 是多少」这个问题本身就问错了**、正解是「当前 $s_t$ 下再加一单位 DR 的 expected value」——让框架贴近 **adaptive experimental design**、非普通 cost-benefit。严格 $m = (\text{type}, \Delta b_m)$、$b' = b + \Delta b_m$、期望 conditional on $\mathcal{D}$。**$m$ 是可执行 batch / 最小预算单元、非无限小的一块钱**（SI fixed cost、DR diminishing returns、FT threshold effects 让极小 $\Delta C$ 上失真）——$MV$ 是 batch-level marginal efficiency、非 infinitesimal derivative。成本向量 $\Delta C(m) = (\Delta C_{\mathrm{real}}, \Delta C_{\mathrm{compute}}, \Delta C_{\mathrm{eng}})$、归一化用影子价格 $\lambda$（**严格 $\lambda^{\mathrm{dual}}_r = \partial V^* / \partial B_r$ 需真正解过 dual 才成立；工程实践里往往是人为 resource weight $w^{\mathrm{resource}}$**）：$C_\lambda(m) = \lambda^\top \Delta C(m)$。
...
**Candidate set 也 state-dependent**：$\mathcal{M}_t = \mathcal{M}(s_t)$——SI 已做完、residual rollout 未过 validation gate、safety 不满足时 online FT 禁、real data 不够时某些 DA pipeline 跑不起来——**直接缩掉 $\mathcal{M}_t$、不是让 $MV$ 变小**；$\mathcal{M}_t^{\mathrm{feasible}} = \mathcal{M}_t^{\mathrm{safe}} \cap \mathcal{M}_t^{\mathrm{budget}}$、**safety 是 action feasibility gate、不是低权重 cost**；$\mathcal{M}_t^{\text{safe}}$ 严格写 $\{m : \mathrm{UCB}_{1-\delta}[\Pr(\text{unsafe} \mid \mathcal{D}_t, m)] \le \alpha\}$、由 posterior predictive / 保守 confidence upper bound 判定、**非经验 failure rate 点估计**。

$$m_{t+1} \;=\; \arg\max_{m \,\in\, \mathcal{M}_t^{\mathrm{feasible}}}\; Q_\lambda^{\mathrm{perf+info}}(m \mid s_t)$$

**$MV$ 保留为跨 intervention 效率读数、不作 decision rule**。二者**回答不同问题**——$MV \to$ efficiency、$Q_\lambda \to$ decision、**不是两个 competing ranking**——极简 toy 展示分岔（**下 toy 里 $\lambda = (3,\ 0.1,\ 1)$ 是示意性 $w^{\mathrm{resource}}$、非解过 dual 求出的 $\lambda^{\mathrm{dual}}$**；$\Delta C = (\text{real-h},\ \text{compute},\ \text{eng-h})$）：
...
$MV$ / $Q_\lambda$ 是 sequential problem 的 **局部决策统计量**；$\lambda_r$ 是当前 state 下资源 $r$ 的**估计影子价格**——越稀缺且越能约束 deployment objective 往往越高、**但非"utilization 高 = $\lambda$ 高"的同义关系**（shadow price = 最优目标对约束 RHS 的边际导数、不是剩余百分比）；只有 intervention 近似无 interaction、成本线性、无 fixed cost 时固定 $\lambda$ 的 greedy 才近似 global——本文不假设这些、$\lambda$ 是随 $t$ 缓慢更新的估计量。这个 ratio 无法从 sim 解析求得、只能用 pilot / ablation / few-shot real eval sequential 地估。五条 caveat：**(i) uncertainty**——真机 $\Delta J$ 噪声大、看 CI / posterior / **LCB**、否则高方差 intervention 因偶然成功被错优先。**(ii) 非线性成本**——SI fixed cost、DR diminishing returns、FT threshold effects。**(iii) 非单调 / 负 MV**——over-randomization、过拟合 FT、错误 residual、negative transfer 可让 $MV$ 为负。**(iv) info value 要真进 local score**——很多 pilot（如 20 min friction ID）即时 $\Delta J \approx 0$、但显著缩小后续 uncertainty set、贡献是 **learning what to do next**；若 global objective 含 VoI、local $Q_\lambda^{\text{perf+info}}$ 也必须含、否则只是两阶段 heuristic。完整 objective $\max (\mathbb{E}[J_{\mathrm{real}}(\pi_T)] + \beta V(\mathcal{D}_T))$。**"信息价值" 严格定义应是 decision value of information**：$\mathrm{VoI}(m) = \mathbb{E}_Y[\max_{m'} \mathbb{E}[U(m')\mid \mathcal{D}, Y]] - \max_{m'} \mathbb{E}[U(m')\mid \mathcal{D}]$——"新 evidence $Y$ 到手后下一步能做出的最好决策比现在好多少"。$V(\mathcal{D}) = -\Pr(\arg\max Q_\lambda$ flips$)$ 只是 **decision-stability proxy**（新实验揭示"$m_B \gg m_A$"时 $\Pr$(flips) 上升、按该式反而算负 info、直觉错误）、**posterior 收窄 ≠ 决策价值**。$MV_{\mathrm{perf}} = \mathbb{E}[\Delta J]/C_\lambda$、$MV_{\mathrm{info}} = \mathbb{E}[\Delta \mathrm{VoI}]/C_\lambda$；**忽略 $MV_{\mathrm{info}}$ 系统性低估 pilot、退化成 exploit-only**。**(v) $\Delta J$ 非天然 causal effect**——observed $\Delta J$ 混 seed / optimizer trajectory / training duration / data ordering / temperature / battery / wear / reset / evaluator 变异 / intervention 本身；**pilot 应做 matched / paired evaluation**（同批 seed、同 held-out real slice、尽量一致 hardware condition）、训练随机性显著时多 seed 把 training variance 纳入 posterior。**$MV$ / $Q_\lambda$ 估的是 intervention protocol 下 incremental value、非无条件因果效应**；$\Delta C(m)$ **必须是全部 incremental cost**（顺带引入的 training compute / optimizer steps / wear / eng effort 一并计入）、否则系统性偏向"训练更久"的 intervention。数值**只在固定 $p_{\mathrm{eval}}$ 下有意义**、换 held-out hardware / object / contact regime、ranking 可能翻盘。

**$MV$ / $Q_\lambda$ 不是固定常数**：$MV_i = MV_i(b_{1:i-1}, \pi_b, D_{\mathrm{real}})$。**可能出现**先 SI 缩 uncertainty set 使 DR $MV$ 下降、先 DR 起点更 robust 使 FT $MV$ 上升——**方向取决于 intervention interaction、本文不假设固定单调**：SI 可能暴露 sim 原本错得更多、DR 可能暴露新 failure mode、FT 可能因 policy 已 robust 反而更难优化、SI 甚至可能诱发 sim overfit。**intervention 之间同时有 complementarity / substitutability / occasional conflict**——是 **resource-constrained sequential experimentation / adaptive allocation**（接近 adaptive experimental design、**不写成 bandit**、无严格 arm / stationary reward / regret 证明）。还有一层反馈：**intervention 不只压低 gap、还改 policy、进而改 policy 对 gap 的敏感度** $S_k^{\mathrm{int}} = S_k^{\mathrm{int}}(\pi)$、$\pi = \pi(m)$：
...
框架有了、再逐条看工具。SI / DR / DA / FT **非同一抽象层级的并列类别**——SI 是 model calibration、DR 是 distribution manipulation、DA 是 representation alignment、FT 是 optimization strategy——并排成"四类方法"会误导四选一、其实是**四个相对独立的 intervention lens**、可组合（本文 analytical decomposition、非领域公认 ontology）：
...
**可微性解决 optimization interface、不解决 model class correctness**——contact model 未表达某真实现象时、再精确梯度也只给"错误模型下的最优参数"；碰撞 / 摩擦 / 接触模式切换往往 **nonsmooth / piecewise-smooth**、即使 $\partial f/\partial\phi$ 存在也不保证梯度稳定、切换处未必优于 derivative-free。SI 两个坑：$p_{\mathrm{real}}(\tau)$ 不可直接访问、只有有限条真机轨迹；参数存在 ≠ 可辨识、identifiability 依赖 excitation 与 sensor observability（质量 / 阻尼 / 刚度在某些激励下产生几乎相同可观测轨迹）。

Residual physics 甜蜜点是 $f_{\mathrm{physics}}$ 仍提供**结构性归纳偏置**、residual 只在目标分布上有限修正——软体（Gao et al., RA-L 2024, 2402.01086）、浮力腿式（Sontakke et al., 2023, 2303.09597）类"主干物理算数、局部稳定残差"场景最好用；若 $f_{\mathrm{physics}}$ 全错、残差独扛整个 dynamics、不如直接学一个 model。$r_\theta$ **并不天然等于"缺失物理"**——unrestricted additive residual 会吞下 sensor bias / actuator error / timing / calibration / reward mismatch 成 **error sponge**、训练分布内拟合好、OOD 失稳；需结构约束（低维 / 稀疏 / 力或加速度尺度 / 物理先验 / 只在特定 contact regime 生效）。$\phi$ 与 $r_\theta$ 还有 **confounding**——残差足够灵活会吸收本应属 $\phi$ 的效应、$\hat\phi$ 失去意义；identifiability 要求两者贡献在数据上可区分（正则化 / scale separation / structural constraints）。
...
更准确：**DR 是对一族环境模型做 population-level 优化**、上式是 risk-neutral average-case baseline abstraction；robust / adversarial DR 换成 $\max_\theta \min_{\phi\in\Phi} J$、CVaR 或其他风险敏感形式。DR 有效条件：$\mathrm{supp}(p_{\mathrm{real}}) \subseteq \mathrm{supp}(p_{\mathrm{DR}})$ 且 real-typical 区域有足够 density。**更根本前提**：real dynamics 可被同一 $\phi$-parameterization 表达；若 sim model class 不含真实现象、$\phi_{\mathrm{real}}$ 无法定义、support 从根上不成立——是 model-class uncertainty、加宽 range 不解决。**主结论**：parameter support 是 design proxy、**deployment-relevant 的是诱导的 policy-conditioned occupancy**——$p_{\mathrm{DR}}(\phi)$ 到 $d_{\mathrm{train}}^{\pi}$ / $d_{\mathrm{real}}^{\pi}$ 的 overlap 决定 downstream transfer；**parameter coverage 必要非充分**。

再一层：**DR 非选 scalar range、而是设计 joint distribution**——$p(\phi_1,\phi_2) \neq p(\phi_1)p(\phi_2)$ 才是常态（payload ↑ 联动 actuator regime、temperature ↑ 联动 motor resistance / friction / battery）、独立 uniform 只是 baseline。回到分配：**randomization 分布要对齐 evaluation 与 objective**、过宽或无关拉低样本效率；但 robust 设定下适当扩大 uncertainty set 反而更稳——**"越宽越保守"并非普遍规律、shape 与对齐才是**。"Adaptive / Automatic DR" 是家族（curriculum / adversarial / posterior-based sampling / performance-driven range adaptation）、共同点是**避免一开始 over-randomize**。

### Axis C — Observation / Representation：domain adaptation 与观测翻译

这条轴处理 $\Delta_{\mathrm{obs}}$、在观测 / 表示层对齐 sim 与 real。**"Representation" 是本文 abstraction**——DA 实际可发生在 input / feature / latent / output / policy / dynamics model 六层；机制包括 feature-level adapter、latent alignment、policy distillation、sim-to-sim canonicalization（image translation / GAN / 扩散只是 input-level 特例；典型如 RCAN, James et al., CVPR 2019, 1812.07252——把随机化 sim 图翻回近似 canonical 干净图喂下游 policy、顺带缝上 DR）。**别把 DA 简化成 image translation**。两条边界：**(a) DA 只是 observation mismatch 子集**——camera intrinsics/extrinsics、temporal sync、sensor bias、depth distortion 更适合 calibration / SI；**(b) task-relevant invariance 才是目标**——只对齐 $z_{\mathrm{sim}}\approx z_{\mathrm{real}}$ 不够、理想保持 $I(z;y_{\mathrm{task}})$ 高、压低 $D(z_{\mathrm{sim}},z_{\mathrm{real}})$、与过宽 DR 抹掉任务信号同源。
...
上面四条轴共享一隐含前提：**$p_{\mathrm{sim}}$ 与 $p_{\mathrm{real}}$ 是两个给定分布**。下面两条路线恰在松动这个前提——非"第五第六种技巧"、是整个问题的 reformulation：**前四条 lens 改变 intervention、WM 与 co-training 改变的是 intervention 所作用的 underlying training substrate**、抽象层级不同、不塞回同一 taxonomy。
...
三者可 hybrid、不必是 $f_{\mathrm{hand}} \rightarrow f_{\mathrm{learned}}$ 的二元替换；读成 simulator replacement 会过度简化。Dreamer（1912.01603）、TD-MPC2（2310.16828）体现这条路——**人工 sim 的 model bias 大到不值得先修**时、WM 提供的是问题本身的改写。DayDreamer（2206.14176）常被误读成"sim 预训练 → real 微调"、更准是展示 **real-interaction-driven 实验路线**——真机上学 WM、用 latent imagination 做 policy improvement。**不依赖手工 sim ≠ model-free**、WM 仍吃假设、只是把 inductive bias 从显式 physics 移到 learned model。

诚实边界："用真实数据学 dynamics" **不等于天然优于仿真**——把"手工建模成本"换成"真机采集 + 模型容量成本"；contact-rich / long-tail / 噪声大场景里学到的 model 常在 OOD 给出**很自信、也很错的想象**——是"手工 sim"与"直接真机 RL"间的又一 trade-off、非终局。**WM net value = predictive utility − model uncertainty risk**：counterfactual selection / rollout-based allocation 时 model uncertainty 必须做 feasibility gate 或 decision penalty、否则加大 sim budget 只是放大 model bias。

### Sim-and-real co-training：把"迁移"重述成 data mixture

Maddukuri et al.（RSS 2025, 2503.24361）的 Sim-and-Real Co-Training 是务实方向。**论文实际报告**：sim + real 混合采样、两平台六视觉操作任务、相对**自带 baseline（train-on-real-only / train-on-sim-only 各自对照）**观测到**平均约 37.9% aggregate relative improvement**——按论文 aggregate metric 跨任务归一化后的 **relative lift**、非 success rate 绝对百分点、不能与 per-task delta 直接比较；引用务必带 baseline 与 aggregation 定义。不做 sim→real 单向迁移、而是一个 recipe 决定两者比例与调度。

**本文解读（非论文证明）**：读成 **data-mixture 问题**——co-training 主要干预变量是 training mixture $p_{\mathrm{train}}=\alpha_{\mathrm{mix}} p_{\mathrm{sim}}+(1-\alpha_{\mathrm{mix}}) p_{\mathrm{real}}$（记 $\alpha_{\mathrm{mix}}$、避免与 shadow price $\lambda$ 冲突）、不是 sim calibration 也非 deployment-time adapter；**$\alpha_{\mathrm{sampling}} \neq \alpha_{\mathrm{effective}}$**——sample 重复、augmentation、importance / loss weighting、curriculum、batch composition 都改 effective training contribution。Mechanistic 分析（Lei et al., arXiv 2026, 2604.13645）指出、**在该工作的 generative robot policy 设置中**、mixture 改变诱发 structured representation alignment 与 importance reweighting——**paper-specific 机制、不外推为"任何 mixture 都 universal 产生这两种效应"**；足以说明"以 mixture 为主抓手、效应跨多维"、不是第五根正交轴。
...
- **安全失败单独统计**：$J_{\mathrm{real}}$ 并列 safety violation / e-stop / intervention / hardware fault / recovery time；**低频事件下"20 次未见 failure" 推不出 $p$ 很低**——用 binomial UCB 或 CVaR 等 **tail-risk measure**、不是 mean ± CI；观测 $X \sim \mathrm{Binomial}(n, p)$、$X = 0$ 只能给 $p$ 的 UCB（Clopper–Pearson 或 Beta posterior $1-\delta$ quantile）、把 $\Pr[\text{unsafe}] \le \alpha$ 变成**"UCB ≤ $\alpha$"的 gate**、非 point-estimate。
...
Spearman=0.95 却把 top-1 选错仍是灾难；反过来 Spearman=0.7 但 top-1 基本不出错、对"选一个能部署的 policy"够用。两者都是 conditional metric。**allocation framework 的结论**：sim fidelity 是 task-of-use dependent、不是 absolute property——换用途（pretrain / exploration / curriculum / safety filter）"哪些误差重要"整个变一遍。$\pi^*_{\mathrm{real}}$ 通常不可得、$R_{\mathrm{select}}$ 与 real-domain learning gap 一样都是 oracle-defined 诊断量、实际用 $J_{\mathrm{real}}(\pi_{\mathrm{best\text{-}observed}}) - J_{\mathrm{real}}(\pi_{\mathrm{sim}})$ 或 Pareto-best proxy。

**更重要的一层**：真实项目通常不要求 sim 精确排序所有 policy、只要求把值得上真机的候选压到可接受集合——**sim → candidate filtering → small real evaluation**、与 allocation philosophy 同构。故 **top-$k$ candidate recall**（真实 top policy 是否进入 sim top-$k$）、**regret@k**、"real best $\in$ sim top-$k$?"应提升到与 ranking 同级。**警惕比 training/eval leakage 更隐蔽的 adaptive selection bias**：sim 若被用来自适应筛选 policy（sim select → real eval → update → 再 sim select）、real evaluation set 应保留**独立 held-out candidates / validation policies**、避免"先用 sim 选、再用同一批被选候选反过来评 sim selection utility"造成 self-confirming 循环——**training / eval leakage ≠ adaptive selection bias**、后者系统性高估 sim ranking 可靠性。**长期做法是维护两个 policy pool**——$\Pi_{\mathrm{adapt}}$ 参与 training / active selection / intervention、$\Pi_{\mathrm{audit}}$ 只做 held-out evaluation 与 sim ranking utility 的定期重估——**audit pool 不参与 adaptive selection**、sim utility 估计才不被 self-confirming 污染。
...
**前两行限定词不能省**：若 uncertainty 来自 **model-class uncertainty**（函数形式表达不了真实现象）、SI 与 DR 未必适用、得先落到 residual / world model / 真机数据。倒数第二行：光 "model unknown" 推不出 WM、判据是 **model uncertainty × real-data budget**——模型类不确定**且**真实交互充足时 learned WM 才合理。最后一行："co-training 兜底"与 allocation 冲突——sim 质量差、real 稀缺、action space / task semantics 不一致时可能负迁移。
...
- **仿真器 model class 本身就差**（$\Delta_{\mathrm{model}}$ 主导且难参数化、软体 / 流体 / 复杂接触）——修 sim 边际效用低、不如 WM 或真机数据。
- **部署分布非常固定**——不需大规模 DR、少量 targeted real FT 往往更划算。
- **sim 不提供独特 coverage / safety / exploration / counterfactual access 时**——$U_{\mathrm{sim}}^{\mathrm{downstream}} < C_{\mathrm{sim}}^{\mathrm{effective}}$：sim 不是"不好"、是没提供 unique utility、opportunity cost 超过收益。

能承认"有时最优是不做 sim-to-real"、正是 allocation framing 应有样子：**不站"仿真"、只站"下一单位预算换回最多真实性能"。** **sequential allocation 需要显式 stopping rule、分三类**：**(a) economic stop**——$\max_{m \in \mathcal{M}_t^{\mathrm{feasible}}} Q_\lambda^{\mathrm{perf+info}}(m \mid s_t) \le 0$、剩余 intervention 扣除机会成本后不再值预算（**decision score 与 stopping rule 必须用同一个 $Q_\lambda$ 变体**，performance-only 就都 performance-only、含 VoI 就都含）；**(b) decision-information stop**——新增 evidence 改变 $\arg\max_m Q_\lambda$ 或 deployment 决策的概率近 0、继续 explore 只买 noise；**(c) safety / feasibility stop**——剩余 candidate 全部落在 $\mathcal{M}_t^{\mathrm{safe}} \cap \mathcal{M}_t^{\mathrm{budget}}$ 之外。任一触发即**停、不默认花完剩余预算**——loop 才能闭环。
...
**Step 5 — 在 feasible 集内按 $Q_\lambda^{\mathrm{perf+info}}$ 选下一份预算**。$m_{t+1} = \arg\max_{m \in \mathcal{M}_t^{\mathrm{safe}} \cap \mathcal{M}_t^{\mathrm{budget}}} Q_\lambda^{\mathrm{perf+info}}(m \mid s_t)$——$\lambda_r$ 是**影子价格估计**（严格 $\lambda^{\mathrm{dual}}$ 需解过 dual、工程里往往是 $w^{\mathrm{resource}}$、稀缺 + 强约束 deployment 才高、非"剩余最少者最大"）、$MV$ 只作跨 intervention 效率读数；VoI 项按上节 $\mathrm{VoI}(m)$ 或退化用 decision-stability proxy、**objective 与 local score 必须一致**。不问"哪个方法最先进"。Safety 走 Step 1 的 $\alpha$ chance constraint、**作 $\mathcal{M}_t^{\mathrm{safe}}$ gate（posterior UCB 判定、非经验 failure rate 点估计）、不进 $Q_\lambda$ 的 cost**。
...
收束：**sim-to-real 不是选一种 transfer technique、而是在当前 belief、不可互换的多种预算和真实评估反馈下、连续决定下一次 intervention**——这是全文理论 spine、其他都是派生。**本文 methodological contribution 不是新 optimizer 或 $\Delta J / \Delta C$ 公式（不是新数学）、而是改变 sim-to-real 的 decision unit**：从「选一种 transfer method」转为「在当前 belief 与多资源约束下选下一次 intervention」、并把 reality gap 与 sim utility 都重述成 policy / evaluation-conditioned 的量。
...
sim-to-real 尚无公认跨任务"哪种方法更强"定量对照——不同任务 / 硬件 / fidelity 上限下结论可能颠倒；上述工作更多是"这类 gap 用这方法可行"的样本、非可外推排序。本文四个 lens 分解、sim utility 三维切分、constrained-allocation 形式化、$\hat S_k^{\mathrm{int}}$ / $MV$ / $Q_\lambda$ 定义都是 **conceptual framework 与作者解读**：sensitivity / ablation / 小规模真实评估估出的 decision statistics、非 sim 解析可求；co-training 读作 data-mixture、WM 读作 model-source replacement、同样不是受控实验证明的结论。
