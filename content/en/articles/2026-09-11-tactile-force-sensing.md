...
Contact perception         (where contact is, area, normal/shear decomposition)
  ↓
State estimation           (contact geometry, slip velocity, stiffness, friction)
  ↓
...
**At least 5 layers of learning / estimation between "signal" and "meaning."** Treating tactile as a *sensor* is not enough; treating it as the full *signal-to-latent chain* is closer to engineering reality.
...
### 3.1 The sensor itself: a multi-dimensional trade-off, not "more resolution is better"

To sense contact, the tactile layer has to be thin, wear-resistant, and survive repeated pressing without breaking. But what actually gives engineers trouble is not "can we build one" — it is *that these goals conflict with each other*. At least 7 dimensions are in tension:
...
Some concrete trade-offs:

- Very high resolution (tens of µm features) usually implies small force range or easy overload.
- Soft sensing skins conform well to curved grasps, but do not always survive high payloads or industrial cycle times.
- High-bandwidth (>1 kHz) designs can capture slip transients, but often pay in long-term drift and calibration stability.
...
The harder layer: **tactile data is heavily embodiment-dependent.** The same "pick up a cup" produces very different signals across robot hands, fingertip materials, and sensors. Vision could form an ImageNet-scale common base not because cameras agree (they don't — resolution, lens, spectrum, intrinsics/extrinsics all differ), but because **pixels / RGB / video as a common representation absorbs those differences**. Tactile does not yet have such a "we're all willing to map our data into it" common representation. This also ties back to [robot data scaling](/en/articles/2026-09-09-robot-data-scaling/): robots are not just short on "action data," they are short on **action data with contact state attached**.

### 3.3 Sim-to-Real: contact-model error is more likely to break a policy than visual-model error
...
| Layer | Vision (fairly mature) | Tactile (current state) |
|---|---|---|
...
That missing chain is exactly the 5-layer stack from §2.1:
...
Task command ────────┼─ Force control        ：specifies "how much force / wrench"
                     │
                     ├─ Impedance control    ：defines motion ↔ force relation
                     │                        (deviation → desired force)
                     │
                     └─ Admittance control   ：from measured force → desired motion
                                              (force → motion)
```
...
- **Impedance control**: make the end-effector *behave like a spring–damper system*: when position deviates from target, generate compliance according to a set force–displacement relation. Direction is **motion → force**.
- **Admittance control**: the reverse — from measured external force, compute a desired position / velocity change and track it. Direction is **force → motion**.

The most useful way to distinguish impedance from admittance is "which side is input, which side is output," not "which is more advanced." In industrial practice they are often mixed, with the choice determined by sensor and actuator bandwidth.
...
Vision / Language / Tactile / Proprioception
 ...
 (pose / velocity)         (stiffness / damping)
       └────────────┬────────────┘
                    ↓
       High-frequency controller (kHz-scale, ~100 µs ~ ms)
                    ↓
                  Motor
                    ↓
                 Contact
...
One physical fact worth calling out as its own line: **the time scale of the VLA and the time scale of contact control are not the same thing.** High-level policy typically updates at 5–30 Hz or slower; low-level controllers close the loop at kHz rates. That middle controller layer is the bridge that turns "decision" into "motor." This is also why, however large the VLA is, it does not automatically become a controller that survives contact disturbances. **"Tactile enters the VLA" ≠ "the VLA is the controller."**
...
### 5.1 Active perception: tactile is not only about "avoiding failure" but also about "exploring"
...
Its scope is broader than "active tactile" — *moving the camera, changing viewpoint, walking around the object, reaching in, pushing* are all active perception; **tactile is one very important realization of it**. The core is: *the robot does not just passively sense the world through sensors; it actively acts to earn the information it wants.* Add this and tactile is upgraded from "a sensor" to "a link in sense — act — re-sense."
...
> **Why, going from raw tactile signal → generalizable contact state → real-time control, do we still not have a unified technology stack comparable to vision?**

This decomposes directly into three segments of the §0 diagram:

1. **Sensor → Raw observation**: hardware trade-off space is too wide; there is no "CMOS image sensor"-equivalent universal form factor (§3.1).
...
3. **Task-relevant latent → Control loop**: the time scales of VLA and contact control are not yet connected; the three segments don't share benchmarks or evaluation metrics (§4.1, §3.4).

*Answering those three clearly moves this article from "tactile popular-science" to "tactile technical review."*
