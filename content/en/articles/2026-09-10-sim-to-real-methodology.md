...
The trajectory distribution is itself **policy-induced** — it changes when $\pi$ changes; it is not an intrinsic property of the environment. What we actually care about is not the distributional gap itself but its **consequence on a task**: the performance gap of the same $\pi$ in the two worlds,

$$\boxed{\;\delta_J(\pi) \;=\; J_{\mathrm{real}}(\pi) \;-\; J_{\mathrm{sim}}(\pi)\;}$$

I call this the **transfer delta**, keeping the sign: if $J$ is success rate and reality is in fact better (the simulator is more conservative, or noise inside sim is harsher than in reality), $\delta_J$ is positive and it feels wrong to call that a "gap." So I separately reserve the magnitude

$$G_J(\pi) \;=\; \big|\,\delta_J(\pi)\,\big|$$

as **performance gap** — that is the semantics used when we talk sensitivity below, so we do not get tangled with sign.
...
$\mathcal{E}$ is the set of evaluation assumptions (which initial states, horizon, reward, constraints you score under), whereas $p_{\mathrm{sim}},p_{\mathrm{real}}$ are the **mechanisms that generate data** — the two should not be conflated. So "our simulator is very accurate" is never a meaningful claim on its own: the same simulator may induce a small gap for a position-control policy and a huge gap for a force-sensitive manipulation policy. **Reality gap is a property of this four-tuple, not of the simulator itself.**
...
**This version removes $\Delta_{\mathrm{opt}}$ (optimization / learning error) from reality gap.** They live at different levels: for **the same fixed policy**, if simulation, observation, and dynamics are all accurate but RL has not converged, $\delta_J$ is actually small while the policy itself is bad — "policy is not well trained" ≠ "sim-to-real gap is large." Stuffing $\Delta_{\mathrm{opt}}$ into $F$ blurs the two back together; the right move is to split them into **two diagnostic quantities**:
...
**These two quantities cannot be unconditionally added and called "deployment loss"**: $\delta_J$ is signed, the two terms have different baselines, and their sum mixes "how sim misleads the current policy" with "how far the current policy is from the real-domain optimum" — that is neither a well-defined deployment loss nor a unified regret. They are **error sources at different levels**, and engineering-wise should be diagnosed separately and attributed separately.

Only for near-operating-point engineering attribution do we locally approximate $F$ as a weighted sum — weights $w_k$ are the task / policy dependent sensitivities, echoing "gap is policy-conditioned." **But be careful about writing $S_k$ as a $\partial J/\partial \Delta_k$ partial-derivative illusion**: latency and friction can be given scalar coordinates, but observation gap is often a high-dimensional distribution divergence, and contact mismatch may not even admit a scalar representation. The more honest writing is to pick an **intervention / perturbation variable $\xi_k$** per mismatch class and estimate sensitivity via a controlled perturbation:

$$\delta_J \;\approx\; \sum_{k} w_k\, \Delta_k, \qquad \hat S_k \;=\; \frac{J_{\mathrm{real}}(\pi;\xi_k+\delta) - J_{\mathrm{real}}(\pi;\xi_k)}{\delta}$$

**$\hat S_k$ is only an auxiliary diagnostic statistic** — it answers "is this class of mismatch worth caring about." The real core decision quantity is $MV(m)$ defined on interventions. The whole narrative therefore becomes **diagnosis → intervention → empirical marginal utility → allocation**, not "measure gap → differentiate → allocate." One further downgrade: $\Delta_{\mathrm{model}}$ and $\Delta_{\mathrm{ctrl}}$ can even compensate each other unidentifiably (if the actuator gain is wrong in sim, the policy quietly re-tunes it via a different command distribution), so both $\hat S_k$ and $MV$ **are not analytically computable quantities from the simulator — they are decision statistics estimated via sensitivity experiments / ablation / small-scale real evaluation**. The framework is not an analytically solvable optimization formula, but an allocation procedure whose marginal returns are estimated via sequential experiments.
...
This ratio is not analytically solvable from the simulator; it must be **sequentially estimated** via pilot experiments / ablation / few-shot real evaluation. And **$MV$ across different interventions is not a fixed constant**: $MV_i = MV_i(b_{1:i-1},\ \pi_b,\ D_{\mathrm{real}})$ — it depends on budget already spent, the current policy, and the real data on hand. The engineering intuition is direct: doing SI first narrows the uncertainty set and DR's $MV$ falls; doing DR first yields a more robust starting point and fine-tune's $MV$ rises; conversely, if you fine-tune on real data first and then add DR, the result can sometimes be more conservative or even conflicting. **Interventions exhibit complementarity, substitutability, and occasional conflict all at once**, so this is not a "one-shot knapsack" but **resource-constrained sequential experimentation / adaptive allocation** (close to adaptive experimental design, but since we do not have strict arms / stationary reward / regret proofs, **please do not write it as a bandit algorithm**).

There is a subtler layer of feedback: **an intervention does not just push the gap down — it changes the policy, and thereby changes the policy's own sensitivity to the gap** — $S_k = S_k(\pi)$ and $\pi = \pi(m)$, so $S_k^{\mathrm{new}} \neq S_k^{\mathrm{old}}$; the loop is not one-directional:
...
**This feedback loop fits the article's allocation thesis better than any new equation**: sim-to-real is not an optimization you solve once, it is a sequential experiment where each round ends by re-estimating for the next.

Mapping each intervention to its primary compressed term and primary budget gives this table — cost is broken along the **budget vector** (SI's bulk is actually real excitation + parameter estimation + instrumentation + simulator engineering + optimization compute, not "simulator fidelity cost"):
...
But honestly add: the "$\times$" here denotes a **combinatorial space**, not orthogonality in the mathematical sense. DR simultaneously touches Model / Observation / Distribution; DA can act on input / feature / latent / policy / output — "DA = Representation axis" is also just an abstraction.

**The tool-selection rule is not "systematic goes to SI, random goes to DR."** That aphorism is fine as memory, but what SI actually does is **fit parameters under an identification objective**:

$$\hat\phi \;=\; \operatorname*{arg\,min}_{\phi}\; \mathcal{L}_{\mathrm{ID}}\big(D_{\mathrm{real}},\ f_{\mathrm{sim}}(\cdot\,;\,\phi)\big)$$

$\mathcal{L}_{\mathrm{ID}}$ can be trajectory prediction, one-step transition error, force/torque residual, state-estimation residual, likelihood, or Bayesian posterior — **many classical SI methods do not perform trajectory-distribution matching at all; they just minimize prediction error**. SI addresses **identifiable, parameterizable model mismatch**, not "everything systematic belongs to it" (actuator gain, latency, friction, and mass may be stochastic processes rather than deterministic biases). Symmetrically, DR addresses **uncertainty that a training distribution can express**. So the more useful partition is not a binary but a continuous spectrum "**point estimate → posterior → robust randomization**":

| Nature of the mismatch | More natural tool |
| --- | --- |
| Parameterizable + identifiable | System Identification (point estimate $\hat\phi$) |
| Parameterizable but only uncertainty is available | Bayesian / posterior SI → posterior-guided DR |
| Parameterizable but hard to identify / high uncertainty | Domain Randomization |
| Hard to express with low-dimensional physics parameters, but a structured residual exists | Residual learning |
...
Here $r_\theta$ is just a **unified notation**: the actual residual need not sit directly on $f$; it can be defined on state transition, force, acceleration, contact impulse, deformation field, or other simulator latents.

There is a make-or-break point that is easily hidden behind the word "differentiable": **differentiability solves the optimization interface, not model class correctness.** If the simulator's contact model simply does not express a real phenomenon, then differentiating through that wrong model as precisely as you like still gives "the optimum under a wrong model." Differentiable simulation lets you estimate $\phi$ better; it does not write $f_{\mathrm{physics}}$ correctly for you — the parts it cannot get right are handed to the residual, or you abandon the premise "first build a trustworthy sim."

SI also has two smaller but practical pitfalls. **First, $p_{\mathrm{real}}(\tau)$ is essentially not directly accessible** — you only have a finite set of real trajectories, so the $\arg\min_\phi$ above is actually run on an empirical estimate $\hat\phi=\arg\min_\phi \sum_i \ell(\tau_i^{\mathrm{sim}}(\phi),\tau_i^{\mathrm{real}})$. **Second, parameters existing ≠ parameters identifiable** — identifiability depends on excitation and sensor observability: mass, damping, and stiffness can produce nearly identical observable trajectories under some excitations and cannot be estimated separately.
...
whereas a classic ensemble is a set of models $\{\pi_1,\dots,\pi_K\}$. A more accurate phrasing is: **DR is a population-level optimization over a family of environment models** — intuitively it resembles ensemble training, but the outcome is one shared policy, not an aggregation of many.

The condition for DR to actually work is **two things stacked together**. **Proxy**: the real parameter distribution has to fall inside DR's support and be densely trained there, roughly $p_{\mathrm{real}}(\phi) \ll p_{\mathrm{DR}}(\phi)$ — if $\phi_{\mathrm{real}}$ technically falls inside the support but lands in an extremely low-probability tail, the policy was essentially never trained there and will still fail. **Main conclusion**: what really determines transfer is not the parameter-space marginal support but whether **the state-action / contact occupancy $d_{\mathrm{real}}^{\pi}(s,a)$ that the policy actually visits under evaluation** sufficiently overlaps with the occupancy induced by the training distribution — **parameter coverage is a necessary proxy, not a sufficient condition for deployment coverage**; friction, mass, and latency can each be within range while the policy still enters a contact mode the simulator has never seen.

One layer further down: **DR is not about picking scalar ranges, it is about designing a joint distribution**. $p(\phi_1,\phi_2) \neq p(\phi_1)p(\phi_2)$ is the norm — payload increase co-varies with actuator regime, temperature increase co-varies with motor resistance / friction / battery. Independent uniform DR is only a convenient baseline, not the natural representation of the real uncertainty set. So the question returns to allocation: **the randomization distribution has to be aligned with the evaluation distribution and the objective** — overly wide or task-irrelevant randomization lowers sample efficiency and forces the policy to compromise across conflicting dynamics; but in robust / adversarial settings, appropriately widening the uncertainty set can actually help. **"Wider is more conservative" is not a universal rule; distribution shape and alignment are what matter.**
...
**Interaction data can come from real, sim, or a mixture** — learned-model route ≠ real-only learning: sim pretrain, real adaptation, sim+real joint, and physics + learned residual are all its instantiations.

To say it precisely: world model **does not cancel the simulator** — it swaps the simulator's role from "hand-specified physics model" to "predictive model learned from interaction data"; what really changes is the **model source**:

$$f_{\mathrm{hand\text{-}designed}} \;\longrightarrow\; f_{\mathrm{learned}}$$

Dreamer (1912.01603) and TD-MPC2 (2310.16828) embody this route. When **the model bias of a hand-crafted simulator is too large to be worth fixing first**, world model offers a rewrite of the sim-to-real problem itself, not a transfer technique under it.

DayDreamer (2206.14176) is often misread as "sim pretraining → real fine-tune." A more accurate statement: **it demonstrates a real-interaction-driven experimental route** — learning a world model directly on a real robot and doing policy improvement via latent imagination, without depending on a handcrafted physics simulator used to pretrain the policy. But this is DayDreamer's choice, not the definition of the world-model family; and **not depending on a handcrafted simulator ≠ model-free** — world-model learning still eats its share of assumptions (representation, architecture, action space, reward, exploration, real data quality), it merely shifts inductive bias from "explicit physics" to "learned world model."

The honest boundary: "learning dynamics from real data" **does not mean it is naturally better than simulation**. It swaps "modeling cost" for "real collection + model capacity cost"; in contact-rich, long-tail, sensor-noisy settings, learned models often produce **confidently wrong imagination** out of distribution. So this is **another trade-off** between "handcrafted sim" and "direct real-world RL," not the end state.

### Sim-and-real co-training: reframing "transfer" as data mixture

Maddukuri et al. (RSS 2025, 2503.24361) proposed Sim-and-Real Co-Training as a pragmatic direction. **What the paper actually reports**: mixing sim and real inside one training run yields an **average of roughly 37.9% relative improvement** across two robot platforms and six visual manipulation tasks (this is not the absolute "success rate jumps from 50% to 87.9%" kind of difference; aggregate statistics depend on how the tasks are defined, and per-task numbers should be checked against the original paper). It does not do one-way sim→real transfer, but is a recipe that fixes the ratio and schedule.

**This article's reading (not the paper's conclusion)** is to push it one step further into a **data-mixture problem**: once reframed that way, DR and real data are no longer substitutes but two sources of $T_R[p_{\mathrm{raw}}]$ on the same sampling distribution (echoing the previous article's "$p_{\mathrm{train}} = T_R[p_{\mathrm{raw}}]$"). Co-training's **primary intervention variable is the training mixture** — $p_{\mathrm{train}}=\lambda\, p_{\mathrm{sim}}+(1-\lambda)\, p_{\mathrm{real}}$ — not simulator calibration and not a deployment-time adapter. But **$\lambda$ is only a sampling-level simplification** — real recipes also change the **effective** training distribution through dataset size, batch construction, importance weighting, augmentation, loss weighting, and curriculum. So do not reduce it to "only the data-distribution axis": follow-up mechanistic analyses (Lei et al., arXiv 2026, 2604.13645) show that changing the mixture also induces **structured representation alignment and importance reweighting**. So this is a route where "mixture is the primary lever, but the effects cut across multiple dimensions," not a fifth axis strictly orthogonal to the previous four.
...
- Explicitly declare whether **task, initial-state, and evaluation distribution** match between sim and real (which is why objective mismatch must be aligned first);
...
- **Do not report means alone**: at least mean ± CI, across multiple random seeds / resets; prefer **paired evaluation** (compare A and B on the same object / initial state / scene / seed), so that environment noise is stripped from the comparison;
- **Report safety failures separately**: mixing emergency stops and grasp failures into a single success rate hides the true deployment cost — $J_{\mathrm{real}}$ should be reported alongside safety violation / e-stop / intervention count / hardware fault / recovery time.
...
In sim it looks like $A > B > C$; on the real robot it flips to $B > C > A$. Here the simulator is not just carrying calibration error — it has **lost model-selection utility** — you would use it to pick out the worst policy. What engineering really dreads is "I trusted sim, and it picked the wrong top-1." So **when the simulator is used for policy / model selection**, look at rank correlation $\rho_{\mathrm{rank}} = \mathrm{Spearman}\big(J_{\mathrm{sim}}(\pi_i),\ J_{\mathrm{real}}(\pi_i)\big)$ together with selection regret:
...
Spearman = 0.95 but a wrong top-1 is still an engineering disaster; conversely, Spearman = 0.7 but top-1 rarely misses may be enough for "pick one deployable policy." Both are **conditional metrics**, only relevant for the "use sim to pick a policy" job.

At this point the **article's second core contribution** (right after the allocation framework) can be stated explicitly: **simulator utility is not one property but three non-interchangeable dimensions** —

| Simulator utility dimension | Typical metric |
| --- | --- |
| Numerical accuracy (calibration / absolute error) | $\mathbb{E}\big[|J_{\mathrm{sim}}(\pi) - J_{\mathrm{real}}(\pi)|\big]$, ECE-style calibration |
| Ranking accuracy | Spearman $\rho_{\mathrm{rank}}$, Kendall $\tau$ |
| Quality of the selected policy (decision quality) | $R_{\mathrm{select}} = J_{\mathrm{real}}(\pi^{*}_{\mathrm{real}}) - J_{\mathrm{real}}(\pi_{\mathrm{sim}})$ |

A simulator can be perfectly calibrated and still pick the wrong policy (narrow distribution); another can be numerically wrong across the board but rank stably and have small selection regret — the three dimensions cannot substitute for each other. So $U_{\mathrm{sim}}$ should not be written as an abstract scalar; it should be expanded into **use-case-specific utility**:
...
Judging fidelity should not target a single policy but be evaluated relative to a **candidate policy family** and a **concrete use case**: $U_{\mathrm{sim}}(\cdot \mid \Pi_{\mathrm{candidate}},\ p_{\mathrm{eval}}^{\mathrm{real}})$. **"Our simulator is very accurate" is never a meaningful claim** — it never answers **which dimension, for which use**.
...
| dynamics uncertainty | medium | scarce | posterior-guided DR / Bayesian SI → DR |
...
That is, **real is used to discover, sim is used to amplify, real is used again to validate** — every unit of budget spent on the segment where its $MV$ is highest.

A common combo is **SI → DR → DA → co-training / fine-tune**: SI calibrates a "80% right" sim, DR widens the family along "hard to name but enumerable" directions, DA handles visual domain shift, and a bit of real data closes it out. **But the arrows are just a schematic of composition, not a fixed workflow.** The real order is determined by the currently dominant gap and by each intervention's marginal utility: when real data is abundant, doing SI first may not be worth it; when vision dominates, DA should move earlier; when SI has very little data, doing a coarse DR first and coming back to calibrate is often more reasonable.
...
This explains a common frustration: why "adding more sim data" sometimes does not help. But to say it precisely: more sim data can also bring wider coverage, more diverse objects, and higher rare-event frequency, so this claim is **conditional** — **when the dominant bottleneck happens to be a support / fidelity mismatch between the simulator and the real evaluation distribution, the marginal value of adding same-distribution samples drops quickly; adding $N$ improves sampling density but cannot automatically create evaluation-relevant coverage or correct model bias.** Instead of "how good is my sim," ask the opening question: **"In which evaluation-relevant directions is my sim close to reality and in which does it fall short? For the ones it falls short on, how sensitive are they, and which budget pushes each most cheaply?"**
...
(The last step re-enters the loop by re-modifying sensitivity and mismatch estimates — see the feedback loop above.)

This chain is not something you analytically run through the simulator to get an answer; it is a **resource-constrained adaptive sequential experimentation framework**: sensitivity and marginal return are estimated via small experiments on real evaluation, and each round decides where the next slice of budget goes. One-sentence wrap-up: **sim-to-real is not a transfer trick; it is a constrained, iteratively estimable allocation problem across model fidelity, training diversity, representation alignment, real interaction, and engineering cost** — that is the same thing as the previous article's "robot data scaling is a sequential data allocation" — only this time, the allocation happens between simulation and reality.
