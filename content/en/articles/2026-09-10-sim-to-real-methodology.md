...
$\hat S_k^{\mathrm{int}}$ is a **local intervention response / sensitivity statistic**, not a true $\partial J / \partial \xi_k$; $S$ is kept only for intuition. $\xi_k$ is **not a natural gap coordinate but an intervention variable defined for a sensitivity experiment**, and many $\xi_k$ are **not directly controllable**. Sensitivity tiers three ways: **direct perturbation** (real-robot latency / friction / appearance), **proxy / surrogate** (via sim or bench, e.g. camera calibration error), **diagnostic ablation** (module / model / dataset swap). **Controlled experimental perturbation, not a derivative of an intrinsic quantity**. Because units across $\xi_k$ are incomparable, **sensitivity is a candidate-generation / prioritization statistic, not the final allocation primitive**; **performance / cost / continuation — $\Delta J(m\mid s_t)$, $\Delta C(m\mid s_t)$, $\mathrm{CVU}(m\mid s_t)$ — are the three core value inputs of the allocation score, while safety and budget define feasibility ($\mathcal{M}_t^{\mathrm{safe}}$, $\mathcal{M}_t^{\mathrm{budget}}$); allocation must return to these four things.** The hierarchy is **descriptor → sensitivity / uncertainty → candidate generation → $(\Delta J,\;\Delta C,\;\mathrm{CVU})$ + feasibility → $Q_\lambda$ → allocation**.
...
The novelty is not "which method is better" but **given current evidence, the expected value of the next intervention**. $m_t^*$ is a one-step / local rule, not a global optimum; when we write down L1 as a global optimization, the decision variable is the **un-starred** action sequence $\{m_t\}$ (writing $\{m_t^*\}$ would conflate the heuristic argmax with the global decision variable). **L1 states the cumulative resource constraint $\sum_t \Delta C_r(m_t \mid s_t) \le B_r$ at the global-feasibility level, while $b_t$ is the same budget's online state representation, evolving via $b_{t+1} = b_t - \Delta C(m_t \mid s_t)$ for local decision use. Writing both is not double-counting — L1 answers "is the whole plan feasible," $b_t$ answers "how much is left for the next local choice," and only the second is a real decision input at step $t$.** Safety splits across two levels: **execution-level** is gated every step through $\mathcal{M}_t^{\mathrm{safe}}$ (event $P_{\mathrm{exec}}$); **deployment-level** appears only as a terminal chance constraint (event $P_{\mathrm{deploy}}$). Full problem: **multi-resource sequential allocation with chance constraint**.
...
$MV$ / $Q_\lambda$ are **local decision statistics**; $\lambda_r$ **under suitable regularity of the value function and constraint qualification can be interpreted as the marginal value of resource $B_r$ at the optimum** (the ideal shadow price), but this article only needs a resource-weight estimate consistent with that meaning: $\lambda_t = \lambda(B_t, \mathcal{D}_t, \pi_t)$ — updates with allocation state, one pivotal experiment can shift it sharply; hereafter we abbreviate uniformly as **resource weights $\lambda_t$**. Fixed-$\lambda$ greedy approximates global optimality only under negligible interaction, linear cost, and no fixed cost. Five caveats: **(i) uncertainty and the decision functional** — main text uses posterior mean; risk-sensitive projects may swap LCB / CVaR-adjusted utility — do not write formula with mean and prose with LCB. **(ii) non-linear cost** — SI fixed, DR diminishing, FT threshold, negative transfer can all drive $MV < 0$. **(iii) continuation-value uplift must enter the local score, but as a heuristic** — we deliberately write $\mathrm{CVU}(m\mid s_t)$ (**one-step, performance-only continuation-value uplift heuristic**) rather than $\mathrm{VoI}$ or a Bellman continuation term. Semantics: **how much the expected net value of the next-best action rises relative to current after executing $m$**, not the current intervention's own future utility. Reasons for the naming: (a) $s_{t+1} = \mathcal{T}(s_t, m_t^*, Y_t)$ carries evidence update, policy change, budget consumption, hardware drift, and candidate-set change — so $\mathrm{CVU}$ measures continuation value of the whole state transition, not pure information value; (b) standard continuation uses $V_{t+1}(s_{t+1})$ while we substitute $\max_{m'} U_0(m'\mid s_{t+1})$, making it an approximation of the next-best one-step performance-only score, not a true value function. A strict information-only definition would require the counterfactual $\mathbb{E}_Y[V(s_{t+1}^{Y})] - V(s_{t+1}^{\varnothing})$; we do not introduce it. Formal definition: $\mathrm{CVU}(m\mid s_t) = \mathbb{E}_{Y \sim p(\cdot\mid s_t,m)}\!\big[\max_{m' \in \mathcal{M}_{t+1}^{\mathrm{feasible}}} U_0(m'\mid s_{t+1})\big] - \max_{m' \in \mathcal{M}_t^{\mathrm{feasible}}} U_0(m'\mid s_t)$, reference utility fixed to $U_0$, no recursion. **$\mathrm{CVU}$ may be negative**: even with $\mu_{\Delta J,t}(m) > 0$, if executing $m$ consumes a lot of budget, $b_{t+1}$ shrinks materially and $\max_{m'} U_0(m'\mid s_{t+1})$ drops — hence $\mathrm{CVU}(m) < 0$. **This is not a bug; the definition intentionally folds resource opportunity cost into the continuation state**, so $\mathrm{CVU}$ measures net continuation uplift relative to current, not the sign of "information itself." **$\mathrm{CVU}$ here is also a *constrained* continuation value under updated safety / feasibility requirements** — information may shrink the feasible set, algebraically looking negative, but the correct reading is "the information revealed a previously incorrect feasibility judgment and prevented a wrong execution," not "information has negative value." For the standard non-negative EVSI, one must additionally allow the decision maker the outside option of "ignoring the signal and sticking with the prior policy." $V(\mathcal{D}) = -\Pr(\arg\max Q_\lambda$ flips$)$ is only a decision-stability proxy; posterior narrowing $\neq$ decision value. **(iv) $\Delta J$ is not a natural causal effect** — matched / paired evaluation; $\Delta C$ includes all incremental cost; $\widehat{\Delta J}_t(m)$ (realized) and $\mu_{\Delta J,t}(m) = \mathbb{E}[\Delta J(m)\mid s_t]$ (belief expectation) are two distinct levels — formulas use $\mu_{\Delta J,t}$, evaluation reports $\widehat{\Delta J}_t$. **(v) Diagnostic-only actions and model-refresh-only actions (which refresh the simulator / surrogate but do not retrain the current policy)** both satisfy immediate $\pi_t^m = \pi_t^{\mathrm{control}}$, $\mu_{\Delta J,t} = 0$ — **note that if the model update itself includes "retrain the policy with the new model," $\Delta J \neq 0$ and the action should be treated as adaptation**; $MV$ is uninformative for these (undefined when incremental cost is also zero); their value is aggregated through $\mathrm{CVU}$ as a unified continuation surrogate (evidence / hypothesis posterior / candidate space / safety feasibility / simulator quality all fold into $\Delta\max U_0$); we do not define an extra reward channel for them in the one-step approximation. Values only meaningful under fixed $p_{\mathrm{eval}}$.
