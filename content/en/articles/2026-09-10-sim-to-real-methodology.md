...
This article does not propose a new algorithm; it proposes a decision framework for comparing and composing existing interventions. The piece converges along three levels: **Diagnosis** ($M_{\mathrm{sim}} \rightarrow p_{\mathrm{sim}}^\pi \rightarrow J_{\mathrm{sim}}$ vs. real — where different, where it matters) · **Intervention** (four lenses Model × Data × Representation × Optimization) · **Allocation** (local score $Q_\lambda^{\mathrm{perf+info}}(m \mid s_t)$, performance-only $Q_\lambda^{\mathrm{perf}}$ as a special case, and efficiency reading $MV(m \mid s_t)$ — where the next unit of resource goes). Then $\text{experiment} \rightarrow \text{real evaluation} \rightarrow \text{update }\mathcal{D}_t \rightarrow \text{next intervention}$ closes the loop; the spine is **Diagnosis → Experiment → Intervention → Allocation → Re-evaluation**, capturing the framing better than the four-lens taxonomy alone.
...
$MV$ / $Q_\lambda$ are **local decision statistics**; $\lambda_r$ is the **estimated shadow price of resource $r$** — scarcer, more binding → higher — but **"high utilization = high $\lambda$" is not a synonym** (shadow price = derivative of the optimum w.r.t. the constraint RHS, not a remaining percentage). Fixed-$\lambda$ greedy approximates global optimality only under negligible interaction, linear cost, and no fixed cost — none assumed here — so $\lambda$ is a slowly-updated estimate over $t$. Neither ratio is analytically computable from the sim; both are estimated **sequentially** via pilots / ablation / few-shot real evaluation. Five caveats: **(i) uncertainty** — real $\Delta J$ is noisy; look at CI / posterior / **LCB**, else a lucky run mis-prioritizes a high-variance intervention. **(ii) non-linear cost** — SI fixed cost, DR diminishing returns, FT threshold effects. **(iii) non-monotonicity / negative MV** — over-randomization, overfitting FT, wrong residual, cross-domain negative transfer can make $MV$ negative. **(iv) information value must enter the local score, not just the objective** — pilots like a 20-min friction ID have $\Delta J \approx 0$ immediately but sharply shrink the downstream uncertainty set; the contribution is **learning what to do next**. If the global objective has VoI, $Q_\lambda^{\mathrm{perf+info}}$ must too, else the framework is only a two-stage heuristic. Full objective: $\max_{\{m_t\}} (\mathbb{E}[J_{\mathrm{real}}(\pi_T)] + \beta V(\mathcal{D}_T))$. **"Information value" strictly means decision VoI**: $\mathrm{VoI}(m) = \mathbb{E}_Y[\max_{m'} \mathbb{E}[U(m') \mid \mathcal{D}, Y]] - \max_{m'} \mathbb{E}[U(m') \mid \mathcal{D}]$. The earlier $V(\mathcal{D}) = -\Pr(\arg\max_m Q_\lambda \text{ flips})$ is a **decision-stability proxy**: an experiment revealing "$m_B \gg m_A$" **raises** flip probability and would score as negative info. **Posterior narrowing $\neq$ decision value**; **ignoring $MV_{\mathrm{info}}$ systematically under-prices pilots and collapses allocation to exploit-only**. **(v) $\Delta J$ is not a natural causal effect** — it mixes seed, optimizer trajectory, training duration, data ordering, temperature / battery / wear, reset and evaluator variation, and the intervention itself; **pilots need matched / paired evaluation** (same seed, held-out slice, consistent hardware); if training randomness is material, multiple seeds fold training variance into the posterior. **$MV$ / $Q_\lambda$ estimate incremental value under an intervention protocol, not an unconditional causal effect.** $\Delta C(m)$ must include **all incremental cost** (compute, extra steps, wear, eng effort), else $MV$ **systematically favors "trains longer" interventions**. Values are **only meaningful under a fixed $p_{\mathrm{eval}}$** — swap held-out hardware / object / contact regime and rankings flip.
...
**Lens disclaimer**: inside the allocation taxonomy I read WM as a reformulation of "model source replacement" — **this article's angle, not a standard definition**. WM's extension is much wider than this section; here we only take the slice "relative to physics sim, the model source has been replaced."
...
**All claims rest on a three-tier evidence chain** — **Level A: mechanism evidence** (friction ID, camera calibration, latency measurement, other directly observable quantities) → **Level B: policy-response evidence** ($\hat S_k^{\mathrm{int}}$, ablation, finite-difference attribution) → **Level C: deployment evidence** (real $\Delta J$, $\Pr(\text{unsafe})$, $Q_\lambda$, $MV$, sim ranking utility). **Tiers cannot substitute** — SI fitting parameters well belongs to Level A and does not imply Level C improvement; this chain is the concrete form of "simulator fidelity is not final truth."
...
$MV$ and $Q_\lambda$ **split duties, not interchangeable** — $MV$ answers "how efficient per unit shadow cost," $Q_\lambda$ answers "worth doing after opportunity cost"; reading both prevents ratio-driven misprioritization while keeping cross-budget comparability. Closing: **sim-to-real is not the choice of a transfer technique; it is the continuous decision, under current belief, non-substitutable budgets, and real-evaluation feedback, of what the next intervention should be** — the article's spine. **The contribution is not a new optimizer or the $\Delta J / \Delta C$ formula; it is a change of sim-to-real's decision unit**: from "pick a transfer method" to "pick the next intervention under current belief and multi-resource constraints," while reframing reality gap and simulator utility as policy / evaluation-conditioned quantities. VoI closes the remaining global-vs-local gap; the three-tier evidence chain keeps "simulator fidelity is not final truth" from being a slogan.
