...
This article does not propose a new algorithm; it proposes a decision framework for comparing and composing existing interventions. The piece converges along three levels: **Diagnosis** ($M_{\mathrm{sim}} \rightarrow p_{\mathrm{sim}}^\pi \rightarrow J_{\mathrm{sim}}$ vs. real — where different, where it matters) · **Intervention** (four lenses Model × Data × Representation × Optimization) · **Allocation** (given current state, budget, and uncertainty — where the next unit of resource goes; the decision score and efficiency reading are formalized later). Then $\text{experiment} \rightarrow \text{real evaluation} \rightarrow \text{update }\mathcal{D}_t \rightarrow \text{next intervention}$ closes the loop; the spine is **Diagnosis → Experiment → Intervention → Allocation → Re-evaluation**, capturing the framing better than the four-lens taxonomy alone. **Three framework contributions + one downstream corollary**: (1) reality gap is a policy-conditioned, task-conditioned consequence, not an intrinsic sim scalar; (2) descriptor / sensitivity is only diagnosis, the decision variable is the intervention; (3) SI / DR / DA / FT are intervention lenses under multi-resource sequential allocation, not mutually exclusive methods; **downstream corollary** — sim evaluation widens from fidelity to downstream utility (prediction / ranking / selection), a consequence of the allocation framework rather than a peer contribution.

Four contributions: **(1) redefining the mismatch** — reality gap is policy- and task-conditioned, not an intrinsic simulator scalar; **(2) separating diagnosis from intervention** — mismatch descriptors / sensitivity are diagnosis, the real decision variable is the intervention; **(3) rewriting method selection as multi-resource sequential allocation** — SI / DR / DA / FT are lenses, not mutually exclusive; **(4) extending simulator evaluation from fidelity to downstream utility** — prediction / ranking / selection quality as a corollary of the allocation framework.
...
Logic is **mechanism → trajectory distribution → performance**: $\mathcal{E}$ collects evaluation assumptions (initial-state / horizon / reward / constraints) — same $M_{\mathrm{sim}}$ can yield a small gap for a position-control policy and a huge gap for force-sensitive manipulation. **This article operationally treats the reality gap as the downstream discrepancy under the four-tuple $(\pi,\mathcal{E},M_{\mathrm{sim}},M_{\mathrm{real}})$, not an intrinsic simulator scalar** — an **operational framing**, not a claim that the community has a unified definition; every later occurrence of "reality gap" uses this notion.
...
$\hat S_k^{\mathrm{int}}$ is a **local intervention response / sensitivity statistic**, not a true $\partial J / \partial \xi_k$; $S$ is kept only for intuition. $\xi_k$ is **not a natural gap coordinate but an intervention variable defined for a sensitivity experiment**, and many $\xi_k$ are **not directly controllable**. Sensitivity tiers three ways: **direct perturbation** (real-robot latency / friction / appearance), **proxy / surrogate** (via sim or bench, e.g. camera calibration error), **diagnostic ablation** (module / model / dataset swap). **Controlled experimental perturbation, not a derivative of an intrinsic quantity** (deliberately avoiding Pearl-style do-calculus). $\hat S_k^{\mathrm{int}}$ is a **diagnostic aid**; the core decision quantity is $Q_\lambda$ (with efficiency reading $MV$) in the next section. **Raw sensitivities across $\xi_k$ are not directly comparable** — ms / dimensionless / degrees / kg share no scale; allocation returns to $\Delta J$ and $\lambda^\top \Delta C$.
...
For budget allocation to be literal, budget is split **continuously** across intervention axes: $b = (b_1, \dots, b_K)$, $b_k$ the amount on intervention $k$ — $b_{\mathrm{SI}} = 2\text{h}$, $b_{\mathrm{DR}} = 10^6$ sim steps, $b_{\mathrm{real}} = 4\text{h}$ real — not 0/1 like "use SI or not." **The deployment objective cannot be mean only**: mean success 90% + catastrophic 1% vs mean 88% + tail ≈ 0 is often **not the same deployment decision** on a real robot — full form is **mean-plus-tail with a safety constraint**:
...
"Where does the next dollar go" is a quantity on interventions, estimated step by step. **The framework is layered around two quantities: local decision score $Q_\lambda$ (the $\arg\max$ object) and efficiency reading $MV$ (cost-normalized, **not the decision rule**)**. Both condition on $s_t = (b_t, \pi_t, \mathcal{D}_t, h_t)$, $h_t$ = hardware condition (battery / temperature / calibration / wear / mode) — explicit when non-stationary, folded into $\mathcal{D}_t$ when drift is small. **"What is DR's $MV$?" is the wrong question**; the right one is "given current $s_t$, what is one more unit of DR worth?" — closer to **adaptive experimental design** than cost-benefit. Formally $m = (\text{type},\ \Delta b_m)$, $b' = b + \Delta b_m$. **$m$ is an executable batch, not an infinitesimal dollar** — SI fixed cost, DR diminishing returns, FT threshold effects break $\Delta J / \Delta C$ at tiny $\Delta C$; **$MV$ is batch-level marginal efficiency**. Cost is a **vector** $\Delta C(m) = (\Delta C_{\mathrm{real}},\ \Delta C_{\mathrm{compute}},\ \Delta C_{\mathrm{eng}})$ collapsed via **shadow prices $\lambda$** (strictly $\lambda^{\mathrm{dual}}_r = \partial V^* / \partial B_r$ requires solving the dual; practice uses a hand-calibrated $w^{\mathrm{resource}}$): $C_\lambda(m) = \lambda^\top \Delta C(m)$. Efficiency reading:
...
But **the real local decision score is the Lagrangian net value and, to match the global objective, must include value of information** — otherwise "global has VoI but local greedy ignores it" leaves a mathematical discontinuity:
...
$U_0$ is a **performance-only reference utility** — $\mathrm{VoI}(\cdot)$ only references $U_0$, not $Q_\lambda^{\mathrm{perf+info}}$ itself, **avoiding the $Q \leftrightarrow \mathrm{VoI}$ self-reference** (a truly self-consistent definition would need a fixed point; we deliberately do not go there). Performance-only $Q_\lambda^{\mathrm{perf}} = U_0$ is a special case; $MV = \mathbb{E}[\Delta J] / \lambda^\top \Delta C$ is the efficiency reading. $Q_\lambda^{\mathrm{perf+info}}$ is a current-state local score, not a Bellman-optimal action value including future option value — an estimable one-step local approximation.

**Candidate set is also state-dependent**: $\mathcal{M}_t = \mathcal{M}(s_t)$ — SI already done, residual rollout not through a validation gate, safety blocks online FT, insufficient real data prevents some DA pipelines — **these shrink $\mathcal{M}_t$ directly, they do not lower $MV$**; feasible subset $\mathcal{M}_t^{\mathrm{feasible}} = \mathcal{M}_t^{\mathrm{safe}} \cap \mathcal{M}_t^{\mathrm{budget}}$, **safety is a feasibility gate, not a low-weight cost**. Strictly $\mathcal{M}_t^{\mathrm{safe}} = \{m : \mathrm{UCB}_{1-\delta}[\Pr(\text{unsafe} \mid \mathcal{D}_t, m)] \le \alpha\}$ — via posterior predictive / conservative confidence upper bound, **not an empirical failure-rate point estimate**. Here $\alpha$ = **allowed failure probability** (a decision variable) and $\delta$ = **statistical confidence tail** (an estimator property) — do not conflate. $m$ is not limited to deployment-changing interventions — **it also covers information-gathering diagnostic experiments** (role ∈ {adaptation, diagnosis, model update}): their immediate $\Delta J \approx 0$, they work through $\mathrm{VoI}$ on subsequent allocation; the **same intervention type at different batch size / training recipe / evaluation protocol counts as a distinct candidate**, since incremental value and cost differ. Decision rule:

$$m_{t+1} \;=\; \arg\max_{m \,\in\, \mathcal{M}_t^{\mathrm{feasible}}}\; Q_\lambda^{\mathrm{perf+info}}(m \mid s_t)$$

**$MV$ stays as an efficiency reading across interventions, not the decision rule.** The two **answer different questions** — $MV \to$ efficiency, $Q_\lambda \to$ decision — **not two competing rankings**; a toy makes the divergence concrete (**$\lambda = (3,\ 0.1,\ 1)$ is a hand-calibrated $w^{\mathrm{resource}}$, not a dual-derived $\lambda^{\mathrm{dual}}$**; $\Delta C = (\text{real-h},\ \text{compute},\ \text{eng-h})$):
...
$MV$ ranks SI first; $Q_\lambda$ ranks DR first — **the ratio reads unit efficiency, the Lagrangian reads net value after opportunity cost**. Assuming $C_\lambda > 0$: **$MV < 0 \Leftrightarrow \mathbb{E}[\Delta J] < 0$** (hurts performance), **$Q_\lambda < 0 \Leftrightarrow \mathbb{E}[\Delta J] < C_\lambda$** (positive but not worth opportunity cost); $0 < MV < 1$ with $Q_\lambda < 0$ is entirely reasonable. DA / FT have $Q_\lambda < 0$ under this $\lambda$ and are dropped by **economic stop**, not "downweighted but still a candidate." Demoting $MV$ to a reporting metric — and separating "harms performance" from "not worth its cost" — is the key clarification.

The novelty is not "which method is better" but **given current evidence, the expected value of the next unit of resource**. $m_{t+1}$ is a **one-step / local rule**, not global optimum; different interventions (30 min SI, $10^6$ DR steps, 100 real trajectories) **do not live in the same intervention space** — the "type" component of $m$ encodes this. Full problem is a **multi-resource sequential allocation with a chance constraint**:
...
$MV$ / $Q_\lambda$ are **local decision statistics**; $\lambda_r$ is the **estimated shadow price of resource $r$** — scarcer, more binding → higher — but **"high utilization = high $\lambda$" is not a synonym** (shadow price = derivative of the optimum w.r.t. the constraint RHS, not a remaining percentage). **$\lambda_t$ updates with the allocation state, not merely with time**: $\lambda_t = \lambda(B_t, \mathcal{D}_t, \pi_t)$ — one pivotal experiment can shift it sharply; calling this "slow" is misleading. Fixed-$\lambda$ greedy approximates global optimality only under negligible interaction, linear cost, and no fixed cost — none assumed here. Neither ratio is analytically computable from the sim; both are estimated **sequentially** via pilots / ablation / few-shot real evaluation. Five caveats: **(i) uncertainty and the decision functional** — real $\Delta J$ is noisy; the main text uses the **posterior mean** as the baseline for $Q_\lambda$, but risk-sensitive projects may swap in LCB, CVaR-adjusted utility, or another conservative decision functional — just do not write the formula with mean and the prose with LCB. **(ii) non-linear cost** — SI fixed cost, DR diminishing returns, FT threshold effects. **(iii) non-monotonicity / negative MV** — over-randomization, overfitting FT, wrong residual, cross-domain negative transfer can make $MV$ negative. **(iv) information value must enter the local score, not just the objective** — diagnostic pilots such as a 20-min friction ID have $\Delta J \approx 0$ immediately but sharply shrink the downstream uncertainty set; the contribution is **learning what to do next**. If the global objective has VoI, $Q_\lambda^{\mathrm{perf+info}}$ must too, else the framework is only a two-stage heuristic. Full objective: $\max_{\{m_t\}} (\mathbb{E}[J_{\mathrm{real}}(\pi_T)] + \beta\,\mathrm{VoI}_T)$. **"Information value" strictly means decision VoI, and its reference utility is $U_0$ (not $Q_\lambda$ itself, avoiding $Q \leftrightarrow \mathrm{VoI}$ recursion)**: $\mathrm{VoI}(m) = \mathbb{E}_Y\!\big[\max_{m'} U_0(m' \mid \mathcal{D}, Y)\big] - \max_{m'} U_0(m' \mid \mathcal{D})$. The earlier $V(\mathcal{D}) = -\Pr(\arg\max_m Q_\lambda \text{ flips})$ is only a **decision-stability proxy**: an experiment revealing "$m_B \gg m_A$" **raises** flip probability and would score as negative info under the proxy. **Posterior narrowing $\neq$ decision value**; **ignoring $MV_{\mathrm{info}}$ systematically under-prices pilots and collapses allocation to exploit-only**. **(v) $\Delta J$ is not a natural causal effect** — it mixes seed, optimizer trajectory, training duration, data ordering, temperature / battery / wear, reset and evaluator variation, and the intervention itself; **pilots need matched / paired evaluation** (same seed, held-out slice, consistent hardware); if training randomness is material, multiple seeds fold training variance into the posterior. **$MV$ / $Q_\lambda$ estimate incremental value under an intervention protocol, not an unconditional causal effect.** $\Delta C(m)$ must include **all incremental cost** (compute, extra steps, wear, eng effort), else $MV$ **systematically favors "trains longer" interventions**. Values are **only meaningful under a fixed $p_{\mathrm{eval}}$** — swap held-out hardware / object / contact regime and rankings flip.

**$MV$ / $Q_\lambda$ are not fixed constants across interventions**: $MV_i = MV_i(b_{1:i-1},\ \pi_b,\ D_{\mathrm{real}})$. **Example interaction patterns**: SI first narrows the uncertainty set so DR's $MV$ falls; DR first yields a more robust starting point so FT's $MV$ rises. **Direction depends on intervention interaction; no fixed monotonic law is assumed** — SI may expose that the sim was more wrong than thought, DR may surface new failure modes, FT may be harder because the policy is already robust, SI can even induce sim overfit. **Interventions exhibit complementarity, substitutability, and occasional conflict simultaneously** — this is **resource-constrained sequential experimentation / adaptive allocation** (close to adaptive experimental design, **not a bandit** — no strict arms / stationary reward / regret guarantees).
...
With this framing, the article is not "which of the four methods is better" but a loop: locate the dominant $\Delta_k$, judge importance via sensitivity, take $m_{t+1} = \arg\max Q_\lambda$ over the feasible set, measure return in real evaluation, decide the next unit.

## Four intervention lenses (more precisely, four relatively independent analytical dimensions)

Framework in place, look at the tools axis by axis. SI, DR, DA, and FT **are not peers at the same abstraction level** — SI is model calibration, DR is training-distribution manipulation, DA is representation alignment, FT is optimization strategy; framing them as "four method families" invites four-way picking, but they are **four relatively independent intervention lenses** that compose (**this article's decomposition, not a community-accepted ontology**):
...
This axis does not chase some "most accurate" $p_{\mathrm{real}}$; it makes the policy robust to a family $\{\phi\}$. Tobin (1703.06907) brought sim grasp detection to the real robot via pure visual randomization; Peng (1710.06537) pushed randomization into dynamics; OpenAI in-hand manipulation (Akkaya et al., 1808.00177) nearly took DR to its extreme — **absorbing difference not through precise calibration but through "a randomization range wide enough."**
...
**All claims rest on a three-tier evidence stack** — $\boxed{\text{A: mechanism}\quad \text{B: policy-response}\quad \text{C: deployment}}$ — A covers friction ID / camera calibration / latency measurement, B covers $\hat S_k^{\mathrm{int}}$ / ablation / finite-difference attribution, C covers real $\Delta J$ / $\Pr(\text{unsafe})$ / $Q_\lambda$ / $MV$ / sim ranking utility. **The three tiers are evidence levels, not a fixed execution order** — real diagnosis cycles between them (a deployment failure → suspicion of latency → back to Level A measurement); but **the tiers cannot substitute for each other** — SI fitting parameters well belongs to A and does not imply C improvement. This is the concrete form of "simulator fidelity is not final truth."
...
**More importantly**, real projects rarely need the sim to precisely rank every policy — they need it to **compress worth-evaluating candidates to an acceptable set**: **sim → candidate filtering → small real evaluation**, structurally identical to the article's allocation philosophy (real discovers, sim amplifies, real re-validates). **top-$k$ candidate recall** (real top in sim top-$k$), **regret@k**, or "real best $\in$ sim top-$k$?" belong at the same tier as ranking, arguably more operational — sim need only include good policies, not rank the tail. Also watch a subtler problem than training/eval leakage — **adaptive selection bias**: if sim adaptively filters (select → real eval → update → select again), keep **independent held-out candidates / validation policies**, else "pick with sim, evaluate sim's utility on the picked set" becomes self-confirming. **Training / eval leakage $\neq$ adaptive selection bias** — the latter systematically overestimates sim ranking reliability. Long-run practice: maintain **two policy pools** — $\Pi_{\mathrm{adapt}}$ for training / active selection / intervention, $\Pi_{\mathrm{audit}}$ for held-out evaluation and periodic re-estimation of sim utility; the audit pool **does not participate in adaptive selection**, so the estimate is not self-confirming.
...
Admitting "sometimes the optimal move is not doing sim-to-real" is exactly what the allocation framing looks like: **it does not take the "simulation" team; it takes the "next unit of budget buys the most real-world performance" team.** Sequential allocation needs an explicit stopping rule with three triggers: **(a) economic stop** — $\max_{m \in \mathcal{M}_t^{\mathrm{feasible}}} Q_\lambda^{\mathrm{perf+info}}(m \mid s_t) \le 0$, remaining interventions not worth their opportunity cost; **score and stopping rule must use the same $Q_\lambda$ variant** — performance-only score → performance-only stop; VoI in objective → VoI in stop. **This is a local one-step stop under the current candidate approximation, not global optimal stopping** — if a strongly complementary intervention portfolio is known a priori, evaluate it as a portfolio candidate before triggering the stop. **(b) decision-information stop** — probability that new evidence flips $\arg\max_m Q_\lambda$ or the deployment decision has fallen close to zero, further exploration only buys noise; **(c) safety / feasibility stop** — remaining candidates fall outside $\mathcal{M}_t^{\mathrm{safe}} \cap \mathcal{M}_t^{\mathrm{budget}}$. Any one triggers **stop, not "spend down the remaining budget by default"**; only this closes the loop.
...
**Step 4 — one-time / initial calibration pilot** to estimate $\Delta J$ and uncertainty (**Step 5 is where sequential adaptive allocation begins**, avoiding the "use a pilot to estimate VoI, then use VoI to pick the pilot" time-order loop). For each candidate intervention, use the smallest feasible sample to estimate $\mathbb{E}[\Delta J]$ with CI / posterior — **the target is to size the effect and rule out clearly low-value interventions, not to preset a fixed sample count** (a low-variance task may need only a handful of trajectories; a fixed "5" contradicts this article's adaptive-allocation stance). **$\Delta J(m)$ is defined against a matched control**: $\Delta J(m) = J_{\mathrm{real}}(\pi_t^{m}) - J_{\mathrm{real}}(\pi_t^{\mathrm{control}})$ — the control bears the same extra training steps / time / seed / data accumulation and only removes this intervention; **$\Delta J$ is incremental deployment utility, not the absolute performance of the post-intervention policy**. Use **matched / paired / block evaluation**: same seed, same held-out real slice, consistent hardware condition; **for systems with noticeable drift** (tire wear, motor temperature, battery decay, calibration), record hardware / calibration state alongside $\mathcal{D}_t$, so drift is not misattributed to the intervention.

**Step 5 — from here on, sequential adaptive allocation**: pick the next budget slice by $Q_\lambda^{\mathrm{perf+info}}$ over the feasible set, each step conditioned on the latest $s_t$ and posterior. $m_{t+1} = \arg\max_{m \in \mathcal{M}_t^{\mathrm{safe}} \cap \mathcal{M}_t^{\mathrm{budget}}} Q_\lambda^{\mathrm{perf+info}}(m \mid s_t)$, $\lambda_r$ the current shadow-price estimate — **strict $\lambda^{\mathrm{dual}}$ requires solving the dual; practice uses a resource weight $w^{\mathrm{resource}}$, high only when scarce **and** strongly binding on deployment, not "smallest remaining budget wins"**; $MV$ stays a cross-intervention efficiency reading; the VoI term follows $\mathrm{VoI}(m)$ above (or falls back to the decision-stability proxy), and **the objective and local score must agree**. **Not "which method is most advanced."** Safety flows through Step 1's $\alpha$ chance constraint as the $\mathcal{M}_t^{\mathrm{safe}}$ **gate** — decided by posterior UCB, not empirical failure rate — **never entering the $Q_\lambda$ cost term.

**Step 6 — Real evaluation → posterior update → back to Step 3.** Update $\mathcal{D}_t \rightarrow \mathcal{D}_{t+1}$, re-estimate $\lambda$, retire falsified hypotheses, add newly observed failure modes, run the next round. **The most-skipped, most-important step** — without posterior update the pipeline degrades to a static checklist.
...
$$\boxed{\begin{aligned}
&\textbf{L1 objective}:\ \max\ \mathbb{E}\big[J_{\mathrm{real}}(\pi_T) + \beta\,\mathrm{VoI}_T\big]\ \text{s.t.}\ \textstyle\sum_t \Delta C_r(m_t)\le B_r,\ \Pr[\text{unsafe} \mid \pi_T]\le \alpha\\
...
&\textbf{L4 decision score}:\ U_0 = \mathbb{E}[\Delta J \mid \mathcal{D}_t] - \lambda^\top \Delta C(m),\;\; Q_\lambda^{\mathrm{perf+info}} = U_0 + \beta\,\mathrm{VoI}\\
