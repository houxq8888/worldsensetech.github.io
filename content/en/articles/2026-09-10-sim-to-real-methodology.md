...
$\hat S_k^{\mathrm{int}}$ is a **local intervention response / sensitivity statistic**, not a true $\partial J / \partial \xi_k$; $S$ is kept only for intuition. $\xi_k$ is **not a natural gap coordinate but an intervention variable defined for a sensitivity experiment**, and many $\xi_k$ are **not directly controllable**. Sensitivity tiers three ways: **direct perturbation** (real-robot latency / friction / appearance), **proxy / surrogate** (via sim or bench, e.g. camera calibration error), **diagnostic ablation** (module / model / dataset swap). **Controlled experimental perturbation, not a derivative of an intrinsic quantity**. Because units across $\xi_k$ are incomparable, **sensitivity is a candidate-generation / prioritization statistic, not the final allocation primitive**; the true allocation primitives are only $\Delta J(m\mid s_t)$, $\Delta C(m\mid s_t)$, $\mathrm{VoI}(m\mid s_t)$. The hierarchy is **descriptor → sensitivity / uncertainty → candidate generation → $\Delta J + \Delta C + \mathrm{VoI}$ → $Q_\lambda$ → allocation**.
...
$$m_t^* \;=\; \arg\max_{m \,\in\, \mathcal{M}_t^{\mathrm{feasible}}}\; Q_\lambda^{\mathrm{perf+info}}(m \mid s_t),\qquad s_{t+1} = \mathcal{T}(s_t,\, m_t^*,\, Y_t)$$
...
| Intervention | $\mu_{\Delta J}$ | real h | compute | eng h | $C_\lambda$ | $MV$ | $Q_\lambda^{\mathrm{perf}}$ |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
$MV$ ranks SI first; $Q_\lambda^{\mathrm{perf}}$ ranks DR first — ratio reads efficiency, Lagrangian reads net value after opportunity cost. **The $Q$ column here is $Q_\lambda^{\mathrm{perf}} = U_0$ (the $\beta = 0$ special case of the formal $Q_\lambda^{\mathrm{perf+info}}$)**; the example deliberately turns off the information channel to isolate efficiency from net value, not to claim information is useless. Domain condition: **when $\lambda^\top \Delta C > 0$**, $MV < 0 \Leftrightarrow \mu_{\Delta J} < 0$; $Q_\lambda^{\mathrm{perf}} < 0 \Leftrightarrow \mu_{\Delta J} < C_\lambda$. DA / FT dropped by economic stop under this $\lambda$.

The novelty is not "which method is better" but **given current evidence, the expected value of the next intervention**. $m_t^*$ is a one-step / local rule, not a global optimum; the budget really enters dynamics via $b_{t+1} = b_t - \Delta C(m_t^*\mid s_t)$, otherwise "allocation" stays symbolic. Full problem: multi-resource sequential allocation with chance constraint.
...
**Step 4 — one-time initial calibration pilot** (Step 5 is where sequential adaptive allocation begins, avoiding the pilot-selection circularity). **For adaptation / model-update actions, estimate an effect distribution** for $\mu_{\Delta J,t}(m)$ (Bayesian implementations realize it as a posterior; frequentist implementations report a CI); **for diagnostic actions, estimate evidence quality / expected $\mathrm{VoI}$** rather than $\Delta J$ — screening a diagnostic by performance gain is a category error. No preset sample count. **$\widehat{\Delta J}_t(m) = J_{\mathrm{real}}(\pi_t^{m}) - J_{\mathrm{real}}(\pi_t^{\mathrm{control}})$** — control bears same extra steps, **same elapsed time (so robot temperature, battery, wear, and other background drift are matched)**, and same seed, only removing this intervention; $\widehat{\Delta J}_t$ is incremental deployment utility, not absolute post-intervention performance; **for diagnostic-only actions $\widehat{\Delta J}_t \equiv 0$, value entirely through $\mathrm{VoI}$**. Matched / paired / block evaluation: same seed, held-out slice, consistent hardware; record drift (tire, motor, battery).

**Step 5 — sequential adaptive allocation**: $m_t^* = \arg\max_{m \in \mathcal{M}_t^{\mathrm{feasible}}(s_t)} Q_\lambda^{\mathrm{perf+info}}(m \mid s_t)$ — $\lambda_r$ is a resource-weight estimate; objective and local score must agree. Cost and budget are simultaneously state-conditioned ($\Delta C(m \mid s_t)$, $b_{t+1} = b_t - \Delta C(m_t^*\mid s_t)$). Safety flows through Step 1's $\alpha$ gate — posterior UCB, never entering $Q_\lambda$ cost.

**Step 6 — Real evaluation → belief update → back to Step 3.** Update $\mathcal{D}_t \rightarrow \mathcal{D}_{t+1}$ (Bayesian implementation realizes this as a posterior; other implementations use moment / CI updates), re-estimate $\lambda$, retire falsified hypotheses, add newly observed failure modes, run the next round. **The most-skipped, most-important step** — without belief update the pipeline degrades to a static checklist.
...
$$\boxed{\begin{aligned}
&\textbf{L1}:\ \max_{\{m_t^*\}}\ \mathbb{E}[J_{\mathrm{real}}(\pi_T)]\quad\text{s.t.}\ \textstyle\sum_t \Delta C_r(m_t^*\mid s_t)\le B_r,\ \Pr[\text{unsafe}\mid s_T]\le \alpha\\
&\textbf{L2}:\ s_t = (b_t,\,\pi_t,\,\mathcal{D}_t,\,h_t),\quad b_{t+1} = b_t - \Delta C(m_t^*\mid s_t)\\
...
&\textbf{L4}:\ Q_\lambda^{\mathrm{perf+info}}(m\mid s_t) = U_0(m\mid s_t) + \beta\,\mathrm{VoI}(m\mid s_t)\quad\text{(one-step look-ahead approx\ of L1)}\\
