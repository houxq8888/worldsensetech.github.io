...
- **report safety failures separately**: $J_{\mathrm{real}}$ should be reported alongside safety violation / e-stop / intervention count / hardware fault / recovery time; **for low-frequency safety events, "zero failures observed in 20 runs" cannot be used to conclude that failure probability is low** — one needs a binomial upper confidence bound or a CVaR-style **tail-risk measure**, not an ordinary mean ± CI. This is what keeps the chance constraint $\Pr[\text{unsafe}] \le \alpha$ from staying purely symbolic.
...
Being willing to admit "sometimes the optimal move is not doing sim-to-real" is exactly what the allocation framing should look like: **it does not take the "simulation" team; it takes the "next unit of budget buys the most real-world performance" team.** In the same spirit, **sequential allocation also needs an internal stopping rule**: when $\max_m MV(m \mid b,\pi,\mathcal{D})$ falls below the opportunity threshold of the remaining budget, or when the posterior uncertainty is already narrow enough not to change the deployment decision, or when the safety constraint is already binding, or when real-direct learning is cheaper — **allocation should stop, rather than default to spending down the remaining budget**; only this closes the loop.
