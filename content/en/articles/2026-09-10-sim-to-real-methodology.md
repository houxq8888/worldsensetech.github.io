...
- **Level 3 — Allocation**: local decision score $Q_\lambda(m \mid s_t)$ and efficiency reading $MV(m \mid s_t)$ — given the current state, budget, and uncertainty, where does the next unit of resource go?
...
So "where does the next dollar go" becomes a quantity defined on interventions that must be estimated step by step in the real world. **The decision framework of this article is layered around two quantities: a local decision score $Q_\lambda$ and an efficiency reading $MV$** — **$Q_\lambda$ is the object actually $\arg\max$-ed; $MV$ is a cost-normalized marginal value / unit-shadow-cost efficiency reading** used for cross-budget / cross-intervention comparison, **not the decision rule**. Both are conditioned on the allocation state $s_t = (b_t, \pi_t, \mathcal{D}_t, h_t)$, where $h_t$ is hardware condition (battery / temperature / calibration / wear / mode) — folded explicitly when the hardware is significantly non-stationary, absorbed into $\mathcal{D}_t$ when drift is small. **The question "what is DR's $MV$?" is already the wrong question**; the right one is "given the current $s_t$, what is the expected value of one more unit of DR?" — which pushes the framework closer to **adaptive experimental design** than to plain cost-benefit analysis. Written rigorously, an intervention is a pair $m = (\text{type},\ \Delta b_m)$, executing it yields $b' = b + \Delta b_m$, and the expectation is explicitly conditional on evidence $\mathcal{D}$. **$m$ is an executable intervention batch / minimal budget unit, not an infinitesimal dollar** — SI has fixed cost, DR has diminishing returns, fine-tuning has threshold effects, and $\Delta J / \Delta C$ misbehaves on very small $\Delta C$; **$MV$ is a batch-level marginal efficiency, not an infinitesimal derivative**. Cost is a **vector** $\Delta C(m) = (\Delta C_{\mathrm{real}},\ \Delta C_{\mathrm{compute}},\ \Delta C_{\mathrm{eng}})$, collapsed to a scalar via **shadow prices $\lambda$** (the marginal value of each binding budget, from LP / KKT or empirical calibration): $C_\lambda(m) = \lambda^\top \Delta C(m)$. The efficiency reading:
...
**Step 5 — Pick the next slice of budget by $Q_\lambda$ over the feasible set.** $m_{t+1} = \arg\max_{m \in \mathcal{M}_t^{\mathrm{safe}} \cap \mathcal{M}_t^{\mathrm{budget}}} Q_\lambda(m \mid s_t)$, with $\lambda_r$ the current shadow-price estimate of resource $r$ (scarce **and** strongly binding on deployment → high $\lambda_r$, not a simple "smallest remaining budget wins"); $MV$ stays as a cross-intervention efficiency reading — **not "which method is most advanced."** Safety flows through Step 1's $\alpha$ chance constraint and acts as the $\mathcal{M}_t^{\mathrm{safe}}$ **gate**, never entering the $Q_\lambda$ cost term.
...
$$\boxed{\begin{aligned}
&\textbf{L1 objective}:\ \max\ \mathbb{E}[J_{\mathrm{real}}(\pi_T)]\ \text{s.t.}\ \textstyle\sum_t \Delta C_r(m_t)\le B_r,\ \Pr[\text{unsafe} \mid \pi_T]\le \alpha\\
...
&\textbf{L4 decision score}:\ Q_\lambda(m \mid s_t) = \mathbb{E}[\Delta J(m) \mid \mathcal{D}_t] - \lambda^\top \Delta C(m)\\
